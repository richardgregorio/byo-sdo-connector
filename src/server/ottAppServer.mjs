...
export async function initOttApp(expressApp) {
  ...
  expressApp.post('/api/sendmessage', upload.single('attachment'), async(req, res) => {
   /* console.log('Reached Fallback sendmessage route', req.body);
    res.json({ ok : true }); */
    console.log('authorizationContext: ', SF_AUTHORIZATION_CONTEXT);
    console.log('authorizationContextType: ', SF_AUTHORIZATION_CONTEXT_TYPE);
    console.log('Body be4 call: ', req.body);
      const responseData = await handleSendmessage(req);
      //res.json(!!responseData.data ? responseData.data : responseData); 
      // above line was failing, so changed for a better logic below
      res.json({
        success: responseData.success,
        id: responseData.id,
        conversationIdentifier: responseData.conversationIdentifier,
        workItemIds: responseData.workItemIds,
        conversationEntryId: responseData.conversationEntryId
      });
  });
...
function handleSendmessage(req) {
  ...
  console.log('settCacheauthContect: ', settingsCache.get("authorizationContextType")); 
